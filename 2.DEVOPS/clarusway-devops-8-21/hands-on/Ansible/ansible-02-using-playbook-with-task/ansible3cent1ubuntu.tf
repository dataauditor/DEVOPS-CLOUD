...
resource "aws_instance" "ansible-control-server" {
  ...
  key_name        = "ugur"
  //  Write your pem file name
  security_groups = ["ansible-secu-gr"]
  tags = {
  ...
  user_data = <<-EOF
              ...
              echo "[webservers]\
              node1 ansible_host=${aws_instance.ansible-worker-server[0].private_ip} ansible_user=ec2-user\
              node2 ansible_host=${aws_instance.ansible-worker-server[1].private_ip} ansible_user=ec2-user\
              [ubuntuservers]\
...
resource "aws_security_group" "ansible-secu-gr" {
  name = "ansible-sec-gr"
  tags = {
    Name = "ansible-secu-group"
  }
  ingress {
    from_port   = 80
    protocol    = "tcp"
