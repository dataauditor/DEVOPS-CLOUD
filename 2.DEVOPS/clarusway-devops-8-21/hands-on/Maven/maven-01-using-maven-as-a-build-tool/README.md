...
java -version
```
sudo chown -R umit maven-experiment/    # wsl'de gerekti. 
sudo apt install maven                  # wsl'de gerekti. 
...
- Show the content of the file ```<maven-experiment>/target/surefire-report/com.clarus.maven.AppTest.txt``` as the output of the test. 
