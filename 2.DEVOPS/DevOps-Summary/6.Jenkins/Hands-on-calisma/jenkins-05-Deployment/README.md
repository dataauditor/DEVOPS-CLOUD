...
- You will `Build Pipeline: Deploy-Application-Staging-Environment` with a pretty graphical presentation.
...
- At `Build Environments` section, select `Delete workspace before build starts` and `Add timestamps to the Console Output` and `Color ANSI Console Output` options.
...
- Go to the `Pipeline-Tomcat-View` and observe the pipeline (`Deploy-Application-Production-Environment` added to the pipeline).
