...
resource "aws_instance" "master" {
  ...
  security_groups      = ["oliver-k8s-master-sec-gr"]
  user_data            = data.template_file.master.rendered
  tags = {
    Name = "oliver-kube-master"
  }
}
...
output "test" {
  value = data.template_file.worker.vars.master-private
}

output "testregion" {
  value = data.template_file.worker.vars.region
}