$ScriptRoot = "C:\Windows\Temp\MSCB2016"

#Apply Windows Server 2016 Member Server Security
Write-Output -ForegroundColor Cyan "Installing Windows Server 2016 Member Server security settings and policies..."
& "$ScriptRoot\Tools\LGPO.exe" /v /g "$scriptroot\GPOs\{088E04EC-440C-48CB-A8D7-A89D0162FBFB}"
& "$ScriptRoot\Tools\LGPO.exe" /v /g "$scriptroot\GPOs\{9C87270F-7704-41D9-A76D-C8B9ADB1794A}"
Write-Output -ForegroundColor Cyan "Windows Server 2016 Member Server Local Policy Applied"

#Apply Windows Defender Local Policy
Write-Output -ForegroundColor Cyan "Installing Windows Defender policies..."
& "$ScriptRoot\Tools\LGPO.exe" /v /g "$scriptroot\GPOs\{4095647A-14FE-4CE4-955A-F2311B0D62D1}"
...
#Server 2016 Member Server Security (registry.pol settings written directly)
Write-Output "Enabling Computer Policies from MSCB for Server 2016"
