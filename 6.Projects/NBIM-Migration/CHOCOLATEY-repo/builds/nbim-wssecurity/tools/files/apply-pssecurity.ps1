...
#Import ScheduledTasks moduleto be used later
...
#Registers a scheduled task to clean up Transcript logs after 14 days
$trigger = New-ScheduledTaskTrigger -At 09:00am -Daily
$action = New-ScheduledTaskAction -Execute 'Powershell.exe' -Argument '-NoProfile -WindowStyle Hidden -command "& Get-ChildItem "$env:SystemDrive\Transcripts"| where-object {$_.LastWriteTime -lt ((get-date).AddDays(-14))} | Select-Object -ExpandProperty FullName | ForEach-Object { Remove-Item $_ -Force -Recurse -Verbose }"'
Register-ScheduledTask -Action $action -Trigger $trigger -User "System" -TaskName "CleanPSTranscripts" -Description "Cleans up PowerShell transcipt logs older than 14 days"
